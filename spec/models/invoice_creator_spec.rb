...
describe InvoiceCreator do
  ...
  before { Timecop.travel(Date.new(Date.current.year, 2)) }
  ...
  context 'when billed quarterly' do
    ...
    specify 'when quarter #2' do
      create_invoice
      Timecop.travel(Date.new(Date.current.year, 5))
    ...
    specify 'when quarter #3' do
      create_invoice
      Timecop.travel(Date.new(Date.current.year, 5)) { create_invoice }
      Timecop.travel(Date.new(Date.current.year, 8))
    ...
    specify 'when quarter #4' do
      ...
      create_invoice
      Timecop.travel(Date.new(Date.current.year, 5)) { create_invoice }
      Timecop.travel(Date.new(Date.current.year, 8)) { create_invoice }
      Timecop.travel(Date.new(Date.current.year, 11))
